...
5. 回主畫面按「開始」。
6. 報告時間到後若仍繼續，畫面會顯示 `+00:01`、`+00:02` 來計算超時。
7. 報告結束時按「暫停/結束本段」，資料會寫入「歷史紀錄」，並自動切到 QA。
8. QA 結束時再按「暫停/結束本段」記錄 QA。
9. 按「下一組」繼續下一組。
...
- `報告結束後自動進入 QA`：預設勾選，報告時間到後會先繼續計算超時；按「暫停/結束本段」結束報告段落後，會自動切到 QA。
