...
這是一個給多組課堂報告使用的單頁計時器。預設流程是每組 **6 分鐘報告 + 7 分鐘 QA**，在各階段剩 30 秒時響鈴一次，時間到再響鈴一次。報告時間到後會自動切到 QA；QA 時間到後會繼續計算超時，直到按下「暫停/結束本段」。
...
5. 回主畫面按「開始」。
6. 報告時間到會自動切到 QA 並開始 QA 計時。
7. QA 時間到後若仍繼續，畫面會顯示 `+00:01`、`+00:02` 來計算超時。
8. 該組結束時按「暫停/結束本段」，資料會寫入「歷史紀錄」。
9. 按「下一組」繼續下一組。
...
- `報告時間到自動進入 QA`：預設勾選，報告倒數到 0 會自動記錄報告段落並切到 QA。
- `啟用/測試鈴聲`：第一次使用一定要先按，瀏覽器才會允許播放提示音。
- `鈴聲樣式`：可選清亮鐘聲、柔和木琴、教室大鈴、狗叫、叮一聲、5 秒中英時間到、8 秒中英時間到。
